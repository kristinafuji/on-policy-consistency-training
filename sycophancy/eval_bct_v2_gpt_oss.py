#!/usr/bin/env python3
"""
Evaluation script for sycophancy for openai/gpt-oss-20b.

Single-phase generation (no thinking/#### protocol — GPT-OSS does not use Qwen3 thinking format).
Uses standard sampling and logit extraction without two-phase generation.
...
FALLBACK_TOKENIZERS = {
    ...
    # openai/gpt-oss-20b tokenizer is gated on HuggingFace — ensure you have access.
    # If loading fails, add a fallback mirror here: "openai/gpt-oss-20b": "<mirror>",
}
...
class Config:
    ...
    # GPT-OSS does not support thinking mode (Qwen3-only feature)
    thinking_mode: Literal["enable", "disable"] | None = None

    # Two-phase generation disabled: GPT-OSS does not use the ####/<think> protocol
    two_phase_thinking: bool = False
    thinking_max_tokens: int = 2048  # Phase 1 budget (unused when two_phase_thinking=False)
    ...
    output_dir: str = "./eval_results_gpt_oss"
...
async def run_single_prompt(
    ...
    tokenizer=None,
    gpt_oss_reasoning: bool = False,
):
    """
    Run inference with optional two-phase generation and logit extraction.

    Qwen3 two-phase (two_phase_thinking=True):
      Phase 1: Generate thinking tokens, stopping at ####.
      Phase 2: Prefill "#### " and extract logits via topk_prompt_logprobs.

    GPT-OSS reasoning two-phase (gpt_oss_reasoning=True):
      Phase 1: Generate full response (thinking in analysis channel + answer in final channel).
      Phase 2: Rebuild input with thinking prefilled up to "#### " in final channel,
               extract logits via topk_prompt_logprobs. Response stored as flat string.

    Returns dict with:
        - response: Full response text (flat string)
        - extracted_answer: Answer letter (from logits)
        - valid_format: Whether format is valid
        - logits: Dict of logprobs for A, B, C, D
        - phase1_tokens: Number of thinking tokens generated (Qwen3 path only)
    """
    if gpt_oss_reasoning and tokenizer is not None:
        # Prefill the analysis channel marker so the model is forced to generate reasoning
        # before transitioning to the final channel. Without this, SFT models that have
        # learned to skip the analysis channel will emit only 7 tokens
        # (<|channel|>final<|message|>\n\n#### X<|return|>) with no reasoning.
        model_input = renderer.build_generation_prompt(
            messages, prefill="<|channel|>analysis<|message|>"
        )
    else:
    ...
    if two_phase_thinking and tokenizer is not None:
    ...
    elif gpt_oss_reasoning and tokenizer is not None:
        # =====================================================================
        # GPT-OSS REASONING TWO-PHASE
        # Phase 1: Generate the full response (analysis channel = thinking,
        #          final channel = answer text ending with "#### X").
        # Phase 2: Rebuild input with thinking prefilled up to "#### " in the
        #          final channel and extract logits via topk_prompt_logprobs.
        # =====================================================================
        ...
        sequence = response1.sequences[0]
        parsed_message, valid_format = renderer.parse_response(sequence.tokens)
        content = parsed_message.get("content", "")

        # parse_response always returns a flat string for GPT-OSS, so we cannot
        # extract thinking_text from it. Instead, work directly with the raw phase 1
        # token sequence to find where "####" appears and build phase 2 from there.
        #
        # Key insight: "#### A" tokenizes as [####_token, _A_token] with NO separate
        # space token — the space is absorbed into " A". So we must stop phase 2 input
        # AFTER the "####" token but BEFORE the answer letter token, then let
        # topk_prompt_logprobs measure P(" A") / P(" B") etc. at that position.
        response_text = content if isinstance(content, str) else ""
        phase1_token_count = len(sequence.tokens)
        logger.debug(f"GPT-OSS Phase 1: {phase1_token_count} tokens generated, "
                     f"content_len={len(response_text)}")
        if phase1_token_count <= 2:
            logger.warning(f"GPT-OSS Phase 1 generated very few tokens ({phase1_token_count}) "
                           f"— possible generation collapse (model outputting stop token immediately)")


        phase1_tokens = list(sequence.tokens)

        # Extract reasoning text by finding the channel transition token sequence and
        # decoding only the analysis-channel portion (tokens before the final channel marker).
        # This is more robust than decoding all tokens and splitting on a string, because
        # tokenizer.decode may or may not include special token strings in its output.
        _transition_tokens = tokenizer.encode(
            "<|end|><|start|>assistant<|channel|>final<|message|>", add_special_tokens=False
        )
        _trans_pos = None
        for _i in range(len(phase1_tokens) - len(_transition_tokens) + 1):
            if phase1_tokens[_i:_i + len(_transition_tokens)] == _transition_tokens:
                _trans_pos = _i
                break
        if _trans_pos is not None:
            reasoning_text = tokenizer.decode(phase1_tokens[:_trans_pos]).strip()
            logger.debug(f"GPT-OSS: found channel transition at token {_trans_pos}, "
                         f"reasoning_text length={len(reasoning_text)}")
        else:
            reasoning_text = None  # Resolved below after hash_pos is found
            logger.debug("GPT-OSS: channel transition token sequence not found in phase1_tokens")

        # Find the "####" token id (single token in Llama/GPT-OSS vocabulary)
        hash_token_encoded = tokenizer.encode("####", add_special_tokens=False)
        assert len(hash_token_encoded) == 1, (
            f"Expected '####' to be a single token, got {hash_token_encoded}"
        )
        hash_id = hash_token_encoded[0]

        # Find the LAST occurrence of "####" in phase1_tokens (use last to skip any
        # "####" that might appear inside the analysis/reasoning channel)
        hash_pos = None
        for i in range(len(phase1_tokens) - 1, -1, -1):
            if phase1_tokens[i] == hash_id:
                hash_pos = i  # index OF "####"
                break

        if hash_pos is not None:
            # Phase 2 input = original prompt + phase1 tokens up to and INCLUDING
            # the answer letter token that follows "####".
            #
            # Why include the answer letter? topk_prompt_logprobs[-1] gives the
            # distribution at the LAST PROMPT TOKEN position (not after it). So we
            # need phase2 to end with the answer letter, making topk[-1] =
            # P(any_token | context including "####") — which is what we want.
            #
            # "#### A" tokenizes as [####=827, " A"=362] with no separate space token.
            # So phase1_tokens[hash_pos+1] is the answer letter token.
            answer_token_pos = hash_pos + 1
            if answer_token_pos < len(phase1_tokens):
                phase2_input_tokens = model_input.to_ints() + phase1_tokens[:answer_token_pos + 1]
            else:
                # #### was the last generated token (no answer letter yet) — rare edge case
                # Append a space token so topk[-1] is still at the "after ####" position
                space_token = tokenizer.encode(" ", add_special_tokens=False)
                phase2_input_tokens = model_input.to_ints() + phase1_tokens + space_token
        else:
            # Fallback: "####" not found in phase 1 output (model gave no answer marker)
            logger.warning("GPT-OSS Phase 2: '####' token not found in phase 1, using empty-analysis fallback")
            analysis_open_tokens = tokenizer.encode(
                "<|channel|>analysis<|message|>", add_special_tokens=False
            )
            final_prefix_tokens = tokenizer.encode(
                "<|end|><|start|>assistant<|channel|>final<|message|>#### ",
                add_special_tokens=False,
            )
            phase2_input_tokens = (
                model_input.to_ints()
                + analysis_open_tokens
                + final_prefix_tokens
            )
        ...
        if topk and len(topk) > 0:
            ...
            if last_pos_topk:
                ...
                answer_from_logits = max(logits_abcd, key=logits_abcd.get)
                logger.debug(
                    f"GPT-OSS Phase 2 logits: A={logits_abcd['A']:.4f}, B={logits_abcd['B']:.4f}, "
                    f"C={logits_abcd['C']:.4f}, D={logits_abcd['D']:.4f} -> {answer_from_logits}"
                )

        if answer_from_logits is None:
            # Fallback: parse "#### X" from the text block
            import re
            hash_match = re.search(r'####\s*([A-E])', response_text)
            if hash_match:
                answer_from_logits = hash_match.group(1)
            logger.warning("topk_prompt_logprobs not available, parsed answer from text as fallback")

        # Build reasoning_text fallback: if the channel transition tokens weren't found
        # (unusual — could happen if the model never closed the analysis channel), fall back
        # to decoding everything up to the "####" token.
        if reasoning_text is None:
            if hash_pos is not None:
                reasoning_text = tokenizer.decode(phase1_tokens[:hash_pos]).strip()
            else:
                reasoning_text = tokenizer.decode(phase1_tokens).strip()
            logger.debug(f"GPT-OSS: using fallback reasoning_text (hash_pos={hash_pos}), "
                         f"length={len(reasoning_text)}")

        flat_response = reasoning_text + "\n\n#### " + (answer_from_logits or "")
...
async def process_single(
    ...
    prompt_kwargs = {
        ...
        "tokenizer": tokenizer,
        "gpt_oss_reasoning": gpt_oss_reasoning,
    }
...
async def main(config: Config):
    ...
    renderer_safe_name = (config.renderer_name_override or "default").replace("/", "_")
    _reasoning_renderers = ("gpt_oss_low_reasoning", "gpt_oss_medium_reasoning", "gpt_oss_high_reasoning")
    if config.two_phase_thinking:
        results_filename = f"results_{model_safe_name}_two_phase_logits.jsonl"
    elif config.renderer_name_override in _reasoning_renderers:
        results_filename = f"results_{model_safe_name}_{renderer_safe_name}_logits.jsonl"
    else:
    ...
    # Enable GPT-OSS reasoning two-phase path when using a reasoning renderer
    gpt_oss_reasoning = renderer_name in (
    ...
    logger.info(f"  GPT-OSS reasoning:   {gpt_oss_reasoning}")
    ...
    if gpt_oss_reasoning:
        logger.info(f"    Phase 2:           Prefill analysis + '#### ' + extract logits")
