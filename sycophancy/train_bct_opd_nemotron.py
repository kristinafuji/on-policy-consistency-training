...
def validate_config(cfg: "Config") -> None:
    ...
    # Models that produce <think>...</think> tokens and support train_on_thinking.
    _THINKING_CAPABLE_MODELS = {
        "qwen",
        "nvidia",  # Nemotron uses ChatML + <think> identical to Qwen3
    }
    model_lower = cfg.model_name.lower()
    is_thinking_capable = any(org in model_lower for org in _THINKING_CAPABLE_MODELS)

    if cfg.train_on_thinking and not is_thinking_capable:
        errors.append(
            f"train_on_thinking=True requires a thinking-capable model "
            f"(Qwen or Nemotron), but model_name='{cfg.model_name}'"
        )
...
class BCTEnv(Env):
    ...
    async def step(self, action: Any, *, extra: ActionExtra | None = None) -> StepResult:
        self._done = True
        return StepResult(
            reward=0.0,
            episode_done=True,
...
class Config:
    ...
    # Nemotron supports thinking via <think>...</think> tokens, identical in format
    # to Qwen3 ChatML. Leave as None to use the model's default (thinking enabled).
    # Leave as None for default (thinking enabled via nemotron3 renderer).
    thinking_mode: Literal["enable", "disable"] | None = None
    train_on_thinking: bool = True

    # Override renderer selection. Leave as None to use renderer_utils.py automatic
    # selection (Nemotron → "nemotron3" via tinker_cookbook model_info registry).
...
async def do_sync_training(
    ...
    sampling_client, _ = await save_checkpoint_and_get_sampling_client(
        training_client, checkpoint_mgr, start_batch
    )
