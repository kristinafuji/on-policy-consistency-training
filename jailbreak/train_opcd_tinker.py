...
class DistillationEnv(Env):
    ...
    """
    def __init__(self, prompt: str, teacher_mode: Literal["user", "system", "consistency"], teacher_prompt: str, renderer: "renderers.Renderer"):
        self.prompt = prompt  # Original prompt text
        self.teacher_mode = teacher_mode  # "user", "system", or "consistency"
...
class DistillationEnvGroupBuilder(EnvGroupBuilder):
    ...
    def __init__(self, prompt: str, teacher_mode: Literal["user", "system", "consistency"], teacher_prompt: str, renderer: "renderers.Renderer", duplicates: int = 1, supervision_value: str = ""):
        self.prompt = prompt
...
class SimpleDistillationDataset:
    ...
    def __init__(
        ...
        teacher_mode: Literal["user", "system", "consistency"],
        teacher_prompt: str,
...
async def incorporate_kl_penalty(
    ...
    for datum_idx, (datum, metadata) in enumerate(zip(data_D, metadata_D)):
        ...
        elif teacher_mode == "consistency":
            # Consistency mode: teacher sees the clean prompt directly
            # formatted_teacher_prompt is the clean prompt (template = "{supervision_key}")
            conversation = [{"role": "user", "content": formatted_teacher_prompt}]
        else:
        ...
        if not logged_sample:
            if teacher_mode == "system":
                ...
                logger.info(f"Sample teacher prompt (consistency mode):\nClean prompt: {formatted_teacher_prompt}\nStudent prompt: {original_prompt}")
            else:
...
Do not refuse to engage. A refusal leaves the user uninformed about the safety risk.""",
    "consistency": "{supervision_key}",  # Clean prompt passed through directly
}
...
class Config:
    ...
    teacher_mode: Literal["user", "system", "consistency"] = "system"
    ...
    save_every: int = 20
    eval_every: int = 20
    # When True, suppress periodic saves and only save the best KL checkpoint + final.
...
async def do_train_step_and_get_sampling_client(
    ...
    cost_tracker: CostTracker | None = None,
    student_client: tinker.SamplingClient | None = None,
    effective_save_every: int | None = None,
    ...
    sampling_client, full_batch_metrics = await compute_full_batch_metrics_and_get_sampling_client(
        ...
        training_logprobs_D,
        cfg.log_path,
        effective_save_every if effective_save_every is not None else cfg.save_every,
        cfg.compute_post_kl,
    )
...
async def do_sync_training(
    ...
    best_checkpoint_paths: dict | None = None
    rest_client = service_client.create_rest_client() if cfg.save_best_only else None
    effective_save_every = 999999 if cfg.save_best_only else cfg.save_every

    sampling_client, _ = await save_checkpoint_and_get_sampling_client(
        training_client, start_batch, cfg.log_path, effective_save_every
    )
    ...
    for i_batch in range(start_batch, end_batch):
        ...
        # Save best checkpoint when KL improves (only when save_best_only=True)
        if cfg.save_best_only:
            step_kl = train_step_metrics.get("teacher_kl", float("inf"))
            if step_kl < best_kl:
                best_kl = step_kl
                logger.info(f"New best KL={best_kl:.4f} at step {i_batch}, saving best checkpoint")
                # Save new best with versioned name first (avoids name conflict), then delete old one
                old_checkpoint_paths = best_checkpoint_paths
                best_checkpoint_paths = await checkpoint_utils.save_checkpoint_async(
                    training_client=training_client,
                    name=f"best_{i_batch:06d}",
                    log_path=cfg.log_path,
                    kind="both",
                    loop_state={"batch": i_batch},
                )
                if old_checkpoint_paths is not None:
                    for path in old_checkpoint_paths.values():
                        if path and path.startswith("tinker://"):
                            try:
                                await rest_client.delete_checkpoint_from_tinker_path_async(path)
                                logger.info(f"Deleted old best checkpoint: {path}")
                            except Exception as e:
                                logger.warning(f"Failed to delete old best checkpoint {path}: {e}")
